...
**구조적 중심:** `FileItem`이 협업 도메인의 허브이며, `SharePermission`·`Comment`·`FileVersion`·`ActivityLog`가 파일을 중심으로 연결된다. `User`/`Admin`은 인증·소유·감사의 주체이고, `StorageService`·`Database`는 인프라 계층으로 도메인 객체와 분리된다.
