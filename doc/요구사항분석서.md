...
| 1.1 | 2026-05-18 | trymimi | U_06·U_09 예외 흐름 세분화, 3.1.2 클래스 관계 요약 추가 | 피드백 반영 |
| 1.2 | 2026-05-18 | trymimi | U_03 업로드 예외 흐름 세분화, Folder–FileItem CRC 관계 표기 통일 | 피드백 반영 |
| 1.3 | 2026-05-18 | trymimi | FileItem `folderId`·FileVersion 관계 표기 정리, U_04 다운로드 예외 흐름 보강 | 정합성 개선 |
| 1.4 | 2026-05-18 | trymimi | Control 클래스·CRC 추가, 순차 다이어그램을 클래스 다이어그램과 정합 | 피드백 반영 |
...
| 구조 관점 분석 | 도메인 클래스, Control 클래스, 클래스 다이어그램, CRC 카드 |
| 행위 관점 분석 | Control 중심 순차 다이어그램, 파일 상태 변화 |
...
| Control 클래스 | 유스케이스 시나리오를 조율하며 UI(Boundary)와 도메인(Entity) 사이에서 권한 확인·협력 객체 호출 순서를 담당하는 제어 계층 클래스 |
| Boundary | 웹 UI·관리자 UI 등 액터와 시스템의 상호작용 경계. 순차 다이어그램에서는 Control에 요청을 전달한다. |
...
- 4.a1: 저장소 용량 또는 조직 할당 쿼터를 초과하면 시스템은 업로드를 거부하고 용량 부족 안내를 표시한다.
- 4.a2: 파일 저장소 연동 중 네트워크 장애 또는 응답 시간 초과 등 일시적 오류가 발생하면 시스템은 부분 업로드를 정리하고 재시도를 안내한다.
- 4.a3: 파일 저장소에 저장하는 중 복구 불가능한 저장 실패가 발생하면 시스템은 기존 데이터가 손상되지 않도록 롤백하고 실패 사유를 표시한다.
- 5.a1: 파일 저장은 완료되었으나 메타데이터 DB 저장에 실패하면 시스템은 저장된 바이너리를 정리하고 메타데이터와 실제 파일 상태의 불일치를 방지한다.
- 5.a2: DB 저장 중 트랜잭션 충돌 또는 동시 업로드로 인한 정합성 오류가 발생하면 시스템은 재시도를 안내하거나 실패 사유를 표시한다.
...
- 3.a2: 파일 저장소 연동 중 네트워크 장애 또는 응답 시간 초과 등 일시적 오류가 발생하면 시스템은 재시도를 안내한다.
- 4.a1: 다운로드 스트림 제공 중 저장소 읽기 오류가 발생하면 시스템은 전송을 중단하고 실패 사유를 표시한다.
...
8. 사용자는 자신이 작성한 댓글의 수정 또는 삭제를 요청한다.
9. 시스템은 요청 사용자가 해당 댓글의 작성자인지 확인한다.
10. 시스템은 댓글을 수정 또는 삭제하고 수정 시간을 갱신한다.
11. 시스템은 댓글 수정/삭제 로그를 기록한다.
...
- 8.a1: 수정/삭제 대상 댓글이 존재하지 않거나 이미 삭제된 경우 시스템은 처리할 수 없음 메시지를 표시한다.
- 9.a1: 요청 사용자가 댓글 작성자가 아닌 경우(타 사용자 댓글 수정/삭제 시도) 시스템은 작업을 차단하고 권한 부족 메시지를 표시한다.
- 10.a1: 수정 시 댓글 내용이 비어 있으면 시스템은 입력 오류를 표시한다.
...
- 2.a1: 동일 이벤트가 중복 감지된 경우(동일 사용자, 대상 리소스, 이벤트 유형, 발생 시각이 일치하거나 멱등 키가 중복) 시스템은 추가 저장을 생략하고 기존 로그를 유지한다.
- 2.a2: 필수 정보(사용자 ID, 대상 파일 ID, 이벤트 유형)가 누락된 경우 시스템은 로그 저장을 건너뛰고 내부 오류 로그에 기록한다.
...
WorkDrive의 핵심 도메인 객체는 사용자, 역할, 파일, 폴더, 공유 권한, 공유 링크, 댓글, 파일 버전, 활동 로그로 구성된다. 파일과 폴더는 조직 협업의 중심 자원이며, 권한과 로그는 보안 및 감사 요구사항을 만족하기 위한 핵심 객체이다. 유스케이스별 시나리오 조율은 `FileUploadControl`, `ShareControl`, `CommentControl`, `VersionControl`, `SearchControl`, `AuditLogControl`이 담당하며, 순차 다이어그램의 참여자 이름과 일치시켜 구조·행위 관점의 정합성을 유지한다.
...
SVG를 열 수 없는 환경에서도 구조를 파악할 수 있도록, 클래스 다이어그램의 주요 관계를 아래와 같이 정리한다.

| 관계 유형 | 출발 클래스 | 대상 클래스 | 설명 |
|-----------|-------------|-------------|------|
| 일반화(Generalization) | Admin | User | 관리자는 사용자의 하위 유형이며, 사용자 관리·정책 설정·감사 로그 조회 기능을 추가로 갖는다. |
| 연관(Association) | User | Role | 사용자는 하나 이상의 역할을 통해 RBAC 권한을 부여받는다. |
| 연관 | User | FileItem | 사용자는 소유(owns) 관계로 파일을 생성·관리한다. |
| 연관 | User | Comment | 사용자는 댓글을 작성(writes)하며, Comment의 `writerId`로 작성자를 식별한다. |
| 연관 | User | ActivityLog | 사용자 행위(업로드, 다운로드, 공유 등)가 활동 로그로 기록된다. |
| 연관 | Folder | FileItem | 폴더는 0개 이상의 파일을 포함(contains)하며, 계층적 분류의 기준이 된다. 파일은 폴더 이동·공유 시에도 독립 식별되므로 집합이 아닌 연관으로 모델링한다. |
| 연관 | FileItem | SharePermission | 파일·폴더별 읽기/수정/댓글 권한이 SharePermission으로 관리된다. |
| 연관 | FileItem | ShareLink | 외부 공유용 링크와 만료 정보가 FileItem에 연결된다. |
| 연관 | FileItem | Comment | 파일 하나에 0개 이상의 협업 댓글이 붙는다. |
| 집합(Aggregation) | FileItem | FileVersion | 파일당 1개 이상의 버전 이력이 유지되며, 복구 시 FileVersion을 참조한다. whole–part는 FileItem 쪽에만 표기한다. |
| 연관 | FileItem | ActivityLog | 파일 단위 접근·수정·공유 이벤트가 ActivityLog에 남는다. |
| 의존(Dependency) | StorageService | FileItem, FileVersion | 실제 바이너리 저장·조회·삭제는 StorageService가 담당하며, 메타데이터는 FileItem/FileVersion이 보유한다. |
| 의존 | Database | User, FileItem | 사용자·파일 메타데이터 등 영속 데이터는 Database를 통해 저장·조회된다. |
| 의존 | FileUploadControl | FileItem, FileVersion, SharePermission, Folder, StorageService, Database, ActivityLog | U_03 업로드 시나리오를 조율한다. |
| 의존 | ShareControl | SharePermission, ShareLink, FileItem, Folder, Database, ActivityLog | U_05 공유 시나리오를 조율한다. |
| 의존 | CommentControl | Comment, SharePermission, FileItem, Database, ActivityLog | U_06 댓글 시나리오를 조율한다. |
| 의존 | VersionControl | FileVersion, FileItem, SharePermission, StorageService, Database, ActivityLog | U_07 버전 복구 시나리오를 조율한다. |
| 의존 | SearchControl | FileItem, SharePermission, Folder, Database | U_08 검색 시나리오를 조율한다. |
| 의존 | AuditLogControl | ActivityLog, Admin, Role, Database | U_10 관리자 로그 조회 시나리오를 조율한다. |

**구조적 중심:** `FileItem`이 협업 도메인의 허브이며, `SharePermission`·`Comment`·`FileVersion`·`ActivityLog`가 파일을 중심으로 연결된다. `User`/`Admin`은 인증·소유·감사의 주체이고, `StorageService`·`Database`는 인프라 계층으로 도메인 객체와 분리된다. **Control 계층**은 유스케이스 단위로 Boundary(UI)와 Entity·인프라 간 메시지 흐름을 연결한다.

#### 3.1.3 Control 클래스와 순차 다이어그램 매핑

| Control 클래스 | 순차 다이어그램 | 관련 유스케이스 | 주요 협력 객체 |
|----------------|-----------------|-----------------|----------------|
| FileUploadControl | 3.3.1 파일 업로드 | U_03 | SharePermission, FileItem, FileVersion, StorageService, ActivityLog |
| ShareControl | 3.3.2 파일 공유 | U_05 | SharePermission, ShareLink, ActivityLog |
| CommentControl | 3.3.3 파일 댓글 | U_06 | SharePermission, Comment, ActivityLog |
| VersionControl | 3.3.4 버전 복구 | U_07 | SharePermission, FileVersion, StorageService, ActivityLog |
| SearchControl | 3.3.5 검색 | U_08 | SharePermission, FileItem |
| AuditLogControl | 3.3.6 관리자 로그 조회 | U_10 | Role, ActivityLog |
...
- Other Associations: FileItem, SharePermission
...
- Other Associations: FileItem, StorageService, ActivityLog
...
#### 3.2.11 FileUploadControl

| 항목 | 내용 |
|------|------|
| Class Name | FileUploadControl |
| ID | C_11 |
| Type | Concrete, Control |
| Description | 파일 업로드 유스케이스(U_03)의 시나리오를 조율한다. |
| Associated Use Case | U_03 |

| Responsibilities | Collaborators |
|------------------|---------------|
| 업로드 권한을 확인한다. | SharePermission, Folder |
| 파일 바이너리 저장과 메타데이터 생성을 순서대로 처리한다. | StorageService, FileItem, FileVersion |
| 업로드 이벤트를 기록한다. | ActivityLog, Database |

Relationships:

- Dependency: SharePermission, FileItem, FileVersion, Folder, StorageService, Database, ActivityLog

#### 3.2.12 ShareControl

| 항목 | 내용 |
|------|------|
| Class Name | ShareControl |
| ID | C_12 |
| Type | Concrete, Control |
| Description | 파일·폴더 공유 유스케이스(U_05)의 시나리오를 조율한다. |
| Associated Use Case | U_05 |

| Responsibilities | Collaborators |
|------------------|---------------|
| 공유 변경 권한을 확인한다. | SharePermission |
| 공유 권한·외부 링크 생성을 처리한다. | SharePermission, ShareLink, FileItem, Folder |
| 공유 이벤트를 기록한다. | ActivityLog, Database |

Relationships:

- Dependency: SharePermission, ShareLink, FileItem, Folder, Database, ActivityLog

#### 3.2.13 CommentControl

| 항목 | 내용 |
|------|------|
| Class Name | CommentControl |
| ID | C_13 |
| Type | Concrete, Control |
| Description | 파일 댓글 관리 유스케이스(U_06)의 시나리오를 조율한다. |
| Associated Use Case | U_06 |

| Responsibilities | Collaborators |
|------------------|---------------|
| 댓글 작성·수정·삭제 권한을 확인한다. | SharePermission |
| 댓글 생성·갱신을 처리한다. | Comment, FileItem |
| 댓글 활동을 기록한다. | ActivityLog, Database |

Relationships:

- Dependency: SharePermission, Comment, FileItem, Database, ActivityLog

#### 3.2.14 VersionControl
...
| ID | C_14 |
| Type | Concrete, Control |
| Description | 파일 버전 복구 유스케이스(U_07)의 시나리오를 조율한다. |
| Associated Use Case | U_07 |

| Responsibilities | Collaborators |
|------------------|---------------|
| 수정 권한을 확인한다. | SharePermission |
| 선택 버전 조회·복구·현재 버전 갱신을 처리한다. | FileVersion, FileItem, StorageService |
| 버전 복구 이벤트를 기록한다. | ActivityLog, Database |

Relationships:

- Dependency: SharePermission, FileVersion, FileItem, StorageService, Database, ActivityLog

#### 3.2.15 SearchControl

| 항목 | 내용 |
|------|------|
| Class Name | SearchControl |
| ID | C_15 |
| Type | Concrete, Control |
| Description | 파일 검색 유스케이스(U_08)의 시나리오를 조율한다. |
| Associated Use Case | U_08 |

| Responsibilities | Collaborators |
|------------------|---------------|
| 사용자 접근 가능 범위를 조회한다. | SharePermission, Role |
| 권한 범위 내에서 파일을 검색한다. | FileItem, Folder, Database |
| 검색 결과를 UI에 반환한다. | FileItem |

Relationships:

- Dependency: SharePermission, FileItem, Folder, Database

#### 3.2.16 AuditLogControl

| 항목 | 내용 |
|------|------|
| Class Name | AuditLogControl |
| ID | C_16 |
| Type | Concrete, Control |
| Description | 관리자 활동 로그 조회 유스케이스(U_10)의 시나리오를 조율한다. |
| Associated Use Case | U_10 |

| Responsibilities | Collaborators |
|------------------|---------------|
| 관리자 권한을 확인한다. | Admin, Role |
| 조건에 맞는 활동 로그를 조회한다. | ActivityLog, Database |
| 조회 결과를 관리자 UI에 반환한다. | ActivityLog |

Relationships:

- Dependency: Admin, Role, ActivityLog, Database

### 3.3 동적 분석

순차 다이어그램은 Boundary(웹 UI·관리자 UI) → **Control** → Entity·인프라 순으로 메시지를 표현한다. Control 클래스 이름은 3.1.1 클래스 다이어그램 및 3.2.11~3.2.16 CRC 카드와 동일하다.

#### 3.3.1 파일 업로드 순차 다이어그램

`FileUploadControl`이 `SharePermission`으로 권한을 확인한 뒤 `StorageService`·`FileItem`·`FileVersion`을 순서대로 호출하고 `ActivityLog`에 기록한다.

![파일 업로드 순차 다이어그램](diagrams/sequence-upload.svg)

#### 3.3.2 파일 공유 순차 다이어그램

`ShareControl`이 공유 권한 확인 후 `SharePermission`·`ShareLink`를 갱신하고 `ActivityLog`에 기록한다.

![파일 공유 순차 다이어그램](diagrams/sequence-share.svg)

#### 3.3.3 파일 댓글 순차 다이어그램

`CommentControl`이 댓글 권한 확인 후 `Comment`를 생성·저장하고 `ActivityLog`에 기록한다.

![파일 댓글 순차 다이어그램](diagrams/sequence-comment.svg)

#### 3.3.4 버전 복구 순차 다이어그램

`VersionControl`이 수정 권한 확인 후 `FileVersion`·`StorageService`를 통해 복구를 수행하고 `ActivityLog`에 기록한다.

![버전 복구 순차 다이어그램](diagrams/sequence-version.svg)

#### 3.3.5 검색 순차 다이어그램

`SearchControl`이 `SharePermission`으로 접근 범위를 확정한 뒤 `FileItem`을 통해 `Database`에서 검색한다.

![검색 순차 다이어그램](diagrams/sequence-search.svg)

#### 3.3.6 관리자 로그 조회 순차 다이어그램

`AuditLogControl`이 `Role`로 관리자 권한을 확인한 뒤 `ActivityLog`를 조회한다.
...
| FileItem | fileId, fileName, folderId, size, extension, storagePath, ownerId | 파일 메타데이터(소속 폴더 포함) |
...
| 외부 요소 | 연동 목적 | 주요 고려사항 |
|-----------|-----------|---------------|
| 파일 저장소 | 실제 파일 저장 및 다운로드 제공 | 용량·쿼터 초과 거부, 일시적 네트워크 오류 시 재시도, 저장 실패 시 롤백 및 메타데이터 정합성 |
...
- 부록 6: `doc/diagrams/*.mmd` - 다이어그램 원본(Mermaid). SVG는 `npx @mermaid-js/mermaid-cli -i <파일>.mmd -o <파일>.svg`로 재생성한다.
